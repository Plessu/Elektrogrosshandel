...
namespace Elektrogrosshandel.GUI.GUI_Menus
{
    internal class GUI_PlaceOrder
    {
        ...
        private static List<Markup> menuItemsAccount = new List<Markup>
        {
                ...
                new Markup("[bold #c0c0c0]  6. Place Order[/]")
        };
        ...
        private static Layout MainMenu()
        {
            ...
            mainMenu["Right"].Update(DisplayInformation().Expand());
        ...
        {
            double bucketValue = Bucket.GetBucketValue(Account.GetActiveBucket(Program.ActiveUser));
            List<Markup> infoLines = new List<Markup>(0);

            infoLines = Bucket.GetArticelsInBucket(Account.GetActiveBucket(Program.ActiveUser));

            infoLines.Add(new Markup($"[bold yellow]----------------------------------------[/]"));
            infoLines.Add(new Markup($"[bold #ffd700]Total Value:[/] [green]{bucketValue.ToString("0.00")} €[/]"));
        ...
        public static Layout ShowPlaceOrderMenu()
        {
            return MainMenu();
        }
    }
